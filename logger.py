    ...
    def __init__(self, filepath="dwell_log.csv"):
        ...
        if not os.path.exists(self.filepath):
            with open(self.filepath, mode='w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["Timestamp", "Track_ID", "Dwell_Time_sec"])
            print(f"📝 新規ログファイルを作成しました: {self.filepath}")

    def record_exit(self, track_id, dwell_time):
        """
        ...
        with open(self.filepath, mode='a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([timestamp, track_id, f"{dwell_time:.1f}"])
        
        # ターミナルにも状況を表示する（動いている感を演出！）
        print(f"💾 [LOG] ID:{track_id} が退出しました。滞在時間: {dwell_time:.1f}秒")